		...
		using (IsolatedStorageFile storage = IsolatedStorageFile.GetUserStoreForApplication()) {
			...
			foreach (GameScreen screen in tempScreensList) {
				...
				if (screen.IsSerializable) {
					...
					string playerValue = screen.ControllingPlayer.HasValue ? screen.ControllingPlayer.Value.ToString() : "";
			...
			using (IsolatedStorageFile storage = IsolatedStorageFile.GetUserStoreForApplication()) {
				...
				using (IsolatedStorageFileStream stream = storage.OpenFile(StateFilename, FileMode.Open)) {
					...
					foreach (XElement screenElem in doc.Root.Elements("GameScreen")) {
						...
						System.Nullable<PlayerIndex> controllingPlayer = !string.IsNullOrEmpty(screenElem.Attribute("ControllingPlayer").Value) ? (PlayerIndex)Enum.Parse(typeof(PlayerIndex), screenElem.Attribute("ControllingPlayer").Value, true) : (System.Nullable<PlayerIndex>)null;
						screen.ControllingPlayer = controllingPlayer;
